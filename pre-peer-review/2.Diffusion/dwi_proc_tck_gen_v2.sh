...
function Help {
    cat <<HELP
Usage:
`basename $0` -m MainDiffusionSequence -r ReversePhaseEncodingSequence -t NumberSimultSliceAcq -o OutputDirectory -a InfantFreeSurferDirectory
...
while getopts "h:m:r:t:o:a:f:p" OPT
  do
  case $OPT in
      h) #help
   Help
...
mrconvert ${main} ${pdir}/dwi.mif -strides -1,-2,+3,+4 -force 
...
slspec=`dirname $(which dwi_proc_tck_gen.sh)`/slspecs/slspec_${tim}_${n_slc}.txt
