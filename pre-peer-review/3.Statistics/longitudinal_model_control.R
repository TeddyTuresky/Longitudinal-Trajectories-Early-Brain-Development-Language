...
source(paste0(code_location, '/long_beh_corr.R'))



dir_in <- '/Users/tht622/Library/CloudStorage/Box-Box/hgse/dev_struct_2023.12.04' # '<location of "reorg" directory>' # should contain reorganized MRI data and non-MRI data (i.e., 'other_dat.csv')
models <- 'log' # model functions: lin, log, quad, asym
measures <- c('volume','wm', 'area', 'thickness', 'meancurv', 'dti_fa', 'dti_md')  # volume, wm, area, thickness, meancurv, dti_fa, dti_md
trks <- c('ARC_L', 'SLF_L') # 'SLF_L', 'ILF_L' # tracts if analyzing diffusion
...
# generate all measures change graph (will use whatever model was last run, except asymptotic)
if (model != 'asym'){
...
long_descriptive_stats(dir_in, measures, hem, trks, covariates, beh_names, med_names)

# test for correlations among behavioral measures using final dataset
long_beh_corr(dir_in, hem, beh_names, med_names)
