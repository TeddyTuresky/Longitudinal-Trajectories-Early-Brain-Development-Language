lin_model_fun <- function(meas, hem, trk, trk_sz, dir_in, other_dat, covariates, regions,
...
dat <- inner_join(mri, other_dat, by = c("id", "timepoint"))
...
if (meas != 'dti_fa' & meas != 'dti_md'){
  qc_parc <- dat[, c('id', 'timepoint', 'qc_parc')]
...
re_cor <- matrix(ncol=1, nrow=length(brain_names))
n_sub_obs <- matrix(ncol=2, nrow=length(brain_names))
n_beh <- matrix(ncol=length(beh_names), nrow=length(brain_names))
est_int <- matrix(ncol=1, nrow=length(brain_names))
est_age <- matrix(ncol=1, nrow=length(brain_names))
est_covs <- matrix(ncol=length(covariates), nrow=length(brain_names))
est_beh <- matrix(ncol=length(beh_names), nrow=length(brain_names))
est_ageXbeh <- matrix(ncol=length(beh_names), nrow=length(brain_names))
est_brain_beh <- matrix(ncol=length(beh_names), nrow=length(brain_names))
se_int <- matrix(ncol=1, nrow=length(brain_names))
se_age <- matrix(ncol=1, nrow=length(brain_names))
se_covs <- matrix(ncol=length(covariates), nrow=length(brain_names))
p_int <- matrix(ncol=1, nrow=length(brain_names))
p_age <- matrix(ncol=1, nrow=length(brain_names))
p_covs <- matrix(ncol=length(covariates), nrow=length(brain_names))
p_beh <- matrix(ncol=length(beh_names), nrow=length(brain_names))
p_ageXbeh <- matrix(ncol=length(beh_names), nrow=length(brain_names))
...
if (meas == 'dti_fa' | meas == 'dti_md'){
  dat1 <- dat[, c("id", "timepoint", "age", covariates)]
} else {
  dat1 <- dat[, c("id", "timepoint", "age", "eTIV", "euler", covariates)]
}
...
if (meas != 'dti_fa' & meas != 'dti_md') {
  ...
  dat1 <- dat1 %>% anti_join( qc_parc1, by=c('id','timepoint'))
}
...
if (vqc_enigma == 1){
  if (brain_name1 == 'bankssts' | brain_name1 == 'superiortemporal'){
    ...
    dat1 <- inner_join(dat1, qc_parcel, by = c("id", "timepoint"))
  }
}
...
while ( nrow(na.omit(dat1_late_diff_qc > 0)) ){
  ...
  dat1_late_diff$dec <- abs(dat1_late_diff$diff/(dat1_late_diff$brain1 - dat1_late_diff$diff))
  ...
  dat1_late_diff_qc_earlier2 <- dat1_late_diff_qc_earlier[, c('id', 'diff_tp')]
  colnames(dat1_late_diff_qc_earlier2)[2] <- 'timepoint'
  dat1_late_diff_qc_later2 <- dat1_late_diff_qc_later[, c('id', 'timepoint')]
  dat1 <- dat1 %>% anti_join( dat1_late_diff_qc_earlier2, by=c('id','timepoint'))
  dat1 <- dat1 %>% anti_join( dat1_late_diff_qc_later2, by=c('id','timepoint'))
...
# Center and scale age and generate quadratic form
dat1$age_act <- dat1$age
sd_age <- sd(dat1$age)
dat1$age <- (dat1$age-40) / sd_age


# Random quadratic intercepts and random slopes model ---- 
FormulaM <- paste0('brain1 ~ 1 + age + ', cov_str, ' + (1 + age|id)')
M1_lin <- lmer(formula=FormulaM, data=dat1, REML=FALSE, 
              control=lmerControl(optimizer='bobyqa', optCtrl=list(maxfun=5e5)))
sum_M1_lin <- summary( M1_lin )
if (meas != 'dti_fa' & meas != 'dti_md') {
  save(M1_lin, file = paste0(dir_in, '/M1_lin_', brain_names[i], '.RData'))
} else {
  save(M1_lin, file = paste0(dir_in, '/M1_lin_', trk, '_', brain_names[i], '.RData'))
}

# for testing models without random slopes
FormulaM <- paste0('brain1 ~ 1 + age + ', cov_str, ' + (1|id)')
M1_lin_no_slope <- lmer(formula=FormulaM, data=dat1, REML=FALSE, 
                        control=lmerControl(optimizer='bobyqa', optCtrl=list(maxfun=5e5)))
if (meas != 'dti_fa' & meas != 'dti_md') {
  save(M1_lin_no_slope, file = paste0(dir_in, '/M1_lin_no_slope_', brain_names[i], '.RData'))
} else {
  save(M1_lin_no_slope, file = paste0(dir_in, '/M1_lin_no_slope_', trk, '_', brain_names[i], '.RData'))
}

n_sub_obs[i,1] <- sum_M1_lin$ngrps[[1]]   
n_sub_obs[i,2] <- nobs(M1_lin)

est_int[i,] <- sum_M1_lin$coefficients[1,1]
est_age[i,] <- sum_M1_lin$coefficients[2,1]
est_covs[i,] <- sum_M1_lin$coefficients[3:(2+length(covariates)),1]

se_int[i,] <- sum_M1_lin$coefficients[1,2]
se_age[i,] <- sum_M1_lin$coefficients[2,2]
se_covs[i,] <- sum_M1_lin$coefficients[3:(2+length(covariates)),2]

p_int[i,] <- sum_M1_lin$coefficients[1,5]
p_age[i,] <- sum_M1_lin$coefficients[2,5]
p_covs[i,] <- sum_M1_lin$coefficients[3:(2+length(covariates)),5]

re_cor[i,] <- attr(sum_M1_lin$varcor$id, which = "correlation")[2]


# Individual growth curves
dat1$brain_pred = predict( M1_lin )

if (meas == 'dti_fa' | meas == 'dti_md'){
  dat_other <- subset( dat1, select = -c(age, age_act, timepoint, brain1, brain_pred))
} else {
  dat_other <- subset( dat1, select = -c(age, age_act, timepoint, brain1, brain_pred, eTIV))
}

synth.dat <- expand.grid( id = unique( dat1$id ),
                         age = seq(-1, 2.5, 0.1))
synth.dat$age_act <- (synth.dat$age * sd_age ) + 40
synth.dat1 <- left_join(synth.dat, dat_other, by = 'id', multiple = 'first')
synth.dat1$brain_pred2 = predict( M1_lin, newdata = synth.dat1, allow.new.levels = TRUE)
...
dat1.typ$id <- -1
dat1.typ$age_act <- (dat1.typ$age * sd_age) + 40
...
# Generate graphs any covariates are significant
for ( k in c(1:length(covariates)) ){
    ...
    dat1.typ.cov_low$age_act <- (dat1.typ.cov_low$age * sd_age) + 40
    
    dat1.typ.cov_high <- synth.dat1.cov_high %>% 
      group_by(age) %>% 
      summarize_at(vars('brain_pred2'), mean)
    dat1.typ.cov_high$id <- -3
    dat1.typ.cov_high$age_act <- (dat1.typ.cov_high$age * sd_age) + 40
    ...
    print(ggplot(dat1, aes(x = age_act, y = brain1, group = id)) +
            ...
            theme(legend.position = "none", axis.text = element_text(size = 11, color = "black"), 
                  ...
                  panel.border = element_blank(),
                  axis.line = element_line(colour = "black")) +
            geom_line(data = dat1.typ.cov_low, aes(x = age_act, y = brain_pred2), col="springgreen", lwd=1, lty=1) +
            geom_line(data = dat1.typ.cov_high, aes(x = age_act, y = brain_pred2), col="lightslateblue", lwd=1, lty=1))
    dev.off() 
}


# random effects parameters
refs <- ranef( M1_lin )$id  # random
...
dat3$age = sage_50
dat3$brain2 <- predict(M1_lin, dat3)
...
for (j in c(1:length(beh_names)))
{
  ...
  dat2.typ.beh_low$age_act <- exp((dat2.typ.beh_low$age))
  
  dat2.typ.beh_high <- synth.dat2.beh_high %>% 
    group_by(age) %>% 
    summarize_at(vars('brain_pred2'), mean)
  dat2.typ.beh_high$id <- -3
  dat2.typ.beh_high$age_act <- exp((dat2.typ.beh_high$age))
  
  dat2.typ.beh_mid <- synth.dat2.beh_mid %>% 
    group_by(age) %>% 
    summarize_at(vars('brain_pred2'), mean)
  dat2.typ.beh_mid$id <- -4
  dat2.typ.beh_mid$age_act <- exp((dat2.typ.beh_mid$age))
  
  if (p_beh[i,j] < 0.05 | p_ageXbeh[i,j] < 0.05){
    pdf(file= paste0(dir_in, '/', hem, '_', meas, '_', brain_name_t, '_', beh_names[[j]], 'split_traj.pdf'), width = 6, height = 6)
    print(ggplot(dat1, aes(x = age, y = brain1, group = id)) +
            ...
            theme(legend.position = "none", axis.text = element_text(size = 14, color = "black"), 
                  ...
                  panel.border = element_blank(),
                  axis.line = element_line(colour = "black")) +
            geom_line(data = dat2.typ.beh_low, aes(x = age_act, y = brain_pred2), color = paste0(meas_color), lwd=2, linetype = "dotted") +
            geom_line(data = dat2.typ.beh_high, aes(x = age_act, y = brain_pred2), color = paste0(meas_color), lwd=2, linetype = "dashed") +
...
# export statistics
stats_sum <<- list('est_covs' = est_covs, 'est_beh' = est_beh, 
                   'est_ageXbeh' = est_ageXbeh, 'se_covs' = se_covs,
                   'p_covs' = p_covs, 'p_beh' = p_beh, 'p_ageXbeh' = p_ageXbeh,
                   'est_brain_beh' = est_brain_beh, 'p_brain_beh' = p_brain_beh,
...
log_model_fun <- function(meas, hem, trk, trk_sz, dir_in, other_dat, covariates, regions,
                          beh_names, rating, dV, ctr_str, use_inf, use3, vqc_enigma, rating_vqc){
  ...
  for (i in c(1:length(brain_names)))
  {
    ...
    # Center and scale age and generate log form
    dat1$age_act <- dat1$age
    dat1$age <- log(dat1$age + 1)
    
    
    # Random quadratic intercepts and random slopes model ---- 
    FormulaM <- paste0('brain1 ~ 1 + age + ', cov_str, ' + (1 + age|id)')
    M1_log <- lmer(formula=FormulaM, data=dat1, REML=FALSE, 
                   control=lmerControl(optimizer='bobyqa', optCtrl=list(maxfun=5e5)))
    sum_M1_log <- summary( M1_log )
    
    if (meas != 'dti_fa' & meas != 'dti_md') {
      save(M1_log, file = paste0(dir_in, '/M1_log_', brain_names[i], '.RData'))
    } else {
      save(M1_log, file = paste0(dir_in, '/M1_log_', trk, '_', brain_names[i], '.RData'))
    }
    
    
    # for testing models without random slopes
    FormulaM <- paste0('brain1 ~ 1 + age + ', cov_str, ' + (1|id)')
    M1_log_no_slope <- lmer(formula=FormulaM, data=dat1, REML=FALSE, 
                            control=lmerControl(optimizer='bobyqa', optCtrl=list(maxfun=5e5)))
    
    if (meas != 'dti_fa' & meas != 'dti_md') {
      save(M1_log_no_slope, file = paste0(dir_in, '/M1_log_no_slope_', brain_names[i], '.RData'))
    } else {
      save(M1_log_no_slope, file = paste0(dir_in, '/M1_log_no_slope_', trk, '_', brain_names[i], '.RData'))
    }
    
    
    n_sub_obs[i,1] <- sum_M1_log$ngrps[[1]]   
    n_sub_obs[i,2] <- nobs(M1_log)
    est_int[i,] <- sum_M1_log$coefficients[1,1]
    est_age[i,] <- sum_M1_log$coefficients[2,1]
    est_covs[i,] <- sum_M1_log$coefficients[3:(2+length(covariates)),1]
    
    se_int[i,] <- sum_M1_log$coefficients[1,2]
    se_age[i,] <- sum_M1_log$coefficients[2,2]
    se_covs[i,] <- sum_M1_log$coefficients[3:(2+length(covariates)),2]
    
    p_int[i,] <- sum_M1_log$coefficients[1,5]
    p_age[i,] <- sum_M1_log$coefficients[2,5]
    p_covs[i,] <- sum_M1_log$coefficients[3:(2+length(covariates)),5]
    
    re_cor[i,] <- attr(sum_M1_log$varcor$id, which = "correlation")[2]
    
    
    # Individual growth curves
    dat1$brain_pred = predict( M1_log )
    ...
    synth.dat <- expand.grid( id = unique( dat1$id ),
                              age = log(seq(0, 120, 1) + 1))
    synth.dat$age_act <- exp(synth.dat$age)
    synth.dat1 <- left_join(synth.dat, dat_other, by = 'id', multiple = 'first')
    synth.dat1$brain_pred2 = predict( M1_log, newdata = synth.dat1, allow.new.levels = TRUE)
    ...
    dat1.typ <- synth.dat1 %>% 
      ...
      summarize_at(vars('brain_pred2'), mean)
    dat1.typ$id <- -1
    dat1.typ$age_act <- exp((dat1.typ$age))
    ...
    for ( k in (1:length(covariates)) ){
      ...
      dat1.typ.cov_low$id <- -2
      dat1.typ.cov_low$age_act <- exp((dat1.typ.cov_low$age))
      ...
      dat1.typ.cov_high <- synth.dat1.cov_high %>% 
        ...
        summarize(brain_mean = mean(brain_pred2), brain_sd = sd(brain_pred2), brain_n = n())
      dat1.typ.cov_high$id <- -3
      dat1.typ.cov_high$age_act <- exp((dat1.typ.cov_high$age))
      ...
      if ( brain_name_t != 'eTIV'){
        ...
        print(ggplot(dat1, aes(x = age_act, y = brain1, group = id)) +
                ...
                scale_y_continuous(name = element_blank()) + # , limits = c(0, 20000), breaks = seq(0, 20000, 10000), labels = function(x) format(x, scientific = TRUE)
    ...
    dat3 <- left_join(du3, dat2, by = 'id', multiple = 'first')
    dat3$age = sage_50
    dat3$brain2 <- predict(M1_log, dat3)
    dat3_beh <- left_join(dat3, behav, by = 'id')
    if (meas != 'dti_fa' & meas != 'dti_md'){
      dat3_beh <- left_join(dat3_beh, dat1_id_euler, by = 'id')
    }
...
quad_model_fun <- function(meas, hem, trk, trk_sz, dir_in, other_dat, covariates, regions,
                           beh_names, rating, dV, ctr_str, use_inf, use3, vqc_enigma, rating_vqc){
  ...
  est_int <- matrix(ncol=1, nrow=length(brain_names))
  est_age <- matrix(ncol=1, nrow=length(brain_names))
  est_age_sq <- matrix(ncol=1, nrow=length(brain_names))
  est_covs <- matrix(ncol=length(covariates), nrow=length(brain_names))
  est_beh <- matrix(ncol=length(beh_names), nrow=length(brain_names))
  est_ageXbeh <- matrix(ncol=length(beh_names), nrow=length(brain_names))
  est_brain_beh <- matrix(ncol=length(beh_names), nrow=length(brain_names))
  ...
  p_int <- matrix(ncol=1, nrow=length(brain_names))
  p_age <- matrix(ncol=1, nrow=length(brain_names))
  p_age_sq <- matrix(ncol=1, nrow=length(brain_names))
  p_covs <- matrix(ncol=length(covariates), nrow=length(brain_names))
  p_beh <- matrix(ncol=length(beh_names), nrow=length(brain_names))
  p_ageXbeh <- matrix(ncol=length(beh_names), nrow=length(brain_names))
  p_brain_beh <- matrix(ncol=length(beh_names), nrow=length(brain_names))
  ...
  for (i in c(1:length(brain_names)))
  {
    ...
    M1_sq <- lmer(formula=FormulaM, data=dat1, REML=FALSE, 
                  control=lmerControl(optimizer='bobyqa', optCtrl=list(maxfun=5e5)))
    sum_M1_sq <- summary( M1_sq )
    if (meas != 'dti_fa' & meas != 'dti_md') {
      save(M1_sq, file = paste0(dir_in, '/M1_sq_', brain_names[i], '.RData'))
    } else {
      save(M1_sq, file = paste0(dir_in, '/M1_sq_', trk, '_', brain_names[i], '.RData'))
    }
    
    
    # for testing models without random slopes
    FormulaM <- paste0('brain1 ~ 1 + age + age_sq + ', cov_str, ' + (1|id)')
    M1_sq_no_slope <- lmer(formula=FormulaM, data=dat1, REML=FALSE, 
                           control=lmerControl(optimizer='bobyqa', optCtrl=list(maxfun=5e5)))
    if (meas != 'dti_fa' & meas != 'dti_md') {
      save(M1_sq_no_slope, file = paste0(dir_in, '/M1_sq_no_slope_', brain_names[i], '.RData'))
    } else {
      save(M1_sq_no_slope, file = paste0(dir_in, '/M1_sq_no_slope_', trk, '_', brain_names[i], '.RData'))
    }
    
    n_sub_obs[i,1] <- sum_M1_sq$ngrps[[1]]   
    n_sub_obs[i,2] <- nobs(M1_sq)
    
    est_int[i,] <- sum_M1_sq$coefficients[1,1]
    est_age[i,] <- sum_M1_sq$coefficients[2,1]
    est_age_sq[i,] <- sum_M1_sq$coefficients[3,1]
    est_covs[i,] <- sum_M1_sq$coefficients[4:(3+length(covariates)),1]
    
    se_int[i,] <- sum_M1_sq$coefficients[1,2]
    se_age[i,] <- sum_M1_sq$coefficients[2,2]
    se_covs[i,] <- sum_M1_sq$coefficients[4:(3+length(covariates)),2]
    
    p_int[i,] <- sum_M1_sq$coefficients[1,5]
    p_age[i,] <- sum_M1_sq$coefficients[2,5]
    p_age_sq[i,] <- sum_M1_sq$coefficients[3,5]
    p_covs[i,] <- sum_M1_sq$coefficients[4:(3+length(covariates)),5]
    
    re_cor[i,] <- attr(sum_M1_sq$varcor$id, which = "correlation")[2]
    
    
    # Individual growth curves
    dat1$brain_pred = predict( M1_sq )
    
    if (meas == 'dti_fa' | meas == 'dti_md'){
      dat_other <- subset( dat1, select = -c(age, age_sq, age_act, timepoint, brain1, brain_pred))
    } else {
      dat_other <- subset( dat1, select = -c(age, age_sq, age_act, timepoint, brain1, brain_pred, eTIV))
    }
    ...
    synth.dat$age_act <- (synth.dat$age * sd_age ) + 40
    synth.dat1 <- left_join(synth.dat, dat_other, by = 'id', multiple = 'first')
    synth.dat1$brain_pred2 = predict( M1_sq, newdata = synth.dat1, allow.new.levels = TRUE)
    ...
    dat3 <- left_join(du3, dat2, by = 'id', multiple = 'first')
    dat3$age = sage_50
    dat3$age_sq = sage_50^2
    dat3$brain2 <- predict(M1_sq, dat3)
    dat3_beh <- left_join(dat3, behav, by = 'id')
    if (meas != 'dti_fa' & meas != 'dti_md'){
      dat3_beh <- left_join(dat3_beh, dat1_id_euler, by = 'id')
    }
...
asym_model_fun <- function(meas, hem, trk, trk_sz, dir_in, other_dat, covariates, regions,
  ...
  vol_models <- list()
  re_cor <- matrix(NA, nrow = length(brain_names), ncol = 1)
  n_beh <- matrix(ncol=length(beh_names), nrow=length(brain_names))
  ...
  for (i in 1:length(brain_names)){
    tryCatch({
      ...
      while ( nrow(na.omit(dat1_late_diff_qc > 0)) ){
        ...
        colnames(dat1_late_diff_qc_earlier2)[2] <- 'timepoint'
        dat1_late_diff_qc_later2 <- dat1_late_diff_qc_later[, c('id', 'timepoint')]
        #print(dat1_late_diff_qc_earlier2)
        #print(dat1_late_diff_qc_later2)
        dat1 <- dat1 %>% anti_join( dat1_late_diff_qc_earlier2, by=c('id','timepoint'))
        dat1 <- dat1 %>% anti_join( dat1_late_diff_qc_later2, by=c('id','timepoint'))
        nloops_late <- nloops_late + 1
      }
      ...
      if ( meas != 'thickness' & meas != 'meancurv') {
        for (tp1 in c('INF', 'TOD')){
          ...
          while ( nrow(na.omit(dat1_diff_qc > 0)) ){
            ...
            colnames(dat1_diff_qc_earlier2)[2] <- 'timepoint'
            dat1_diff_qc_later2 <- dat1_diff_qc_later[, c('id', 'timepoint')]
            #print(dat1_diff_qc_earlier2)
            #print(dat1_diff_qc_later2)
            dat1 <- dat1 %>% anti_join( dat1_diff_qc_earlier2, by=c('id','timepoint'))
            dat1 <- dat1 %>% anti_join( dat1_diff_qc_later2, by=c('id','timepoint'))
            nloops <- nloops + 1
          }
      ...
      if (meas == 'dti_fa' | meas == 'dti_md'){
        dat_other <- subset( dat1, select = -c(age, timepoint, brain1, brain_pred))
      } else {
        dat_other <- subset( dat1, select = -c(age, timepoint, brain1, brain_pred, eTIV))
      }
