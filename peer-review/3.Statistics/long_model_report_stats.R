long_model_report_stats <- function(meas, hem, trk_sz, dir_in, regions,
  ...
  if (is.null(covariates_interaction) == 1){ 
    ...
    all_covs <- c(covariates, paste0('x', covariates_interaction))
  }
  ...
  p_fdr_cov <- p.adjust(as.numeric(c(x_p_covs)), 'fdr') # na.omit(
  mat_cov <- matrix(p_fdr_cov, nrow = length(regions), ncol = length(all_covs))
  print(mat_cov)
  
  print('pFDR: int-behavior')
  p_fdr_beh <- p.adjust(as.numeric(c(x_p_beh)), 'fdr') # na.omit(
  mat_beh <- matrix(p_fdr_beh, nrow = length(regions), ncol = length(beh_names))
  print(mat_beh)
  
  print('pFDR: slope-behavior')
  p_fdr_ageXbeh <- p.adjust(as.numeric(c(x_p_ageXbeh)), 'fdr') # na.omit(
  mat_ageXbeh <- matrix(p_fdr_ageXbeh, nrow = length(regions), ncol = length(beh_names))
  print(mat_ageXbeh)
  
  print('pFDR: 50mo-behavior')
  p_fdr_brain_beh <- p.adjust(as.numeric(c(x_p_brain_beh)), 'fdr') # na.omit(
  ...
  # Index FDR-corrected regions with binary arrays
  mat_beh_fdr_bin <- mat_beh < 0.05
  mat_ageXbeh_fdr_bin <- mat_ageXbeh < 0.05
  
  # Index estimates and FDR-corrected regions and behavioral measures by brain measure and hemisphere
  # and their corresponding uncorrected p-values
  for (fdr_cor in (0:1)){
    if (fdr_cor == 0){
      indx_cov <- which(x_est_covs < 2, arr.ind = T)
      indx_beh <- which(x_est_beh < 2, arr.ind = T)
      indx_ageXbeh <- which(x_est_ageXbeh < 2, arr.ind = T)
      indx_brain_beh <- which(x_est_brain_beh < 2, arr.ind = T)
    } else if (fdr_cor == 1){
    ...
    if (fdr_cor == 0){
      if (nrow(indx_cov_txt) > 0){
        ...
        indx_cov_txt[,2] <- all_covs[indx_cov[,2]]
        indx_cov_txt[,3] <- x_est_covs[indx_cov]
        indx_cov_txt[,4] <- 'covs'
      }
      ...
      if (nrow(indx_beh_txt) > 0){
        ...
        indx_beh_txt[,3] <- x_est_beh[indx_beh]
        indx_beh_txt[,4] <- 'int'
      }
      ...
      if (nrow(indx_ageXbeh_txt) > 0){
        ...
        indx_ageXbeh_txt[,3] <- x_est_ageXbeh[indx_ageXbeh]
      ...
      if (nrow(indx_brain_beh_txt) > 0){
        ...
        indx_brain_beh_txt[,3] <- x_est_brain_beh[indx_brain_beh]
        indx_brain_beh_txt[,4] <- '50mo'
      }
      
      # consolidate continuous estimates for ggseg visualizations
      indx_est <<- rbind(indx_cov_txt, indx_beh_txt, indx_ageXbeh_txt, indx_brain_beh_txt)
      ...
      if (nrow(indx_brain_beh_txt) > 0){
        indx_brain_beh_txt[,1] <- regions[indx_brain_beh[,1]]
        indx_brain_beh_txt[,2] <- beh_names[indx_brain_beh[,2]]
        indx_brain_beh_txt[,3] <- x_p_brain_beh[indx_brain_beh]
        indx_brain_beh_txt[,4] <- '50mo'
      }
    
      # consolidate continuous fdr stats for ggseg visualizations
      indx_fdr <<- rbind(indx_cov_txt, indx_beh_txt, indx_ageXbeh_txt, indx_brain_beh_txt)
      
    } 
  }
  
  # consolidate binary fdr stats for ggseg visualizations
  indx_fdr_bin <<- list('indx_fdr_int' = mat_beh_fdr_bin, 
                        'indx_fdr_slope' = mat_ageXbeh_fdr_bin)

  print('Numbers of participants and observations for brain models')
  print(stats_sum$n_sub_obs)
  
  print('Numbers of observations removed for brain models')
  print(stats_sum$n_obs_removed)
  
  print('Numbers of participants for behavioral analyses')
  print(stats_sum$n_beh)
  
  print('Minimum number of participants and observations by measure for brain models')
  print(apply(stats_sum$n_sub_obs,2,min))
  
  print('Minimum number of participants for behavioral analyses')
...
long_model_report_stats_asym <- function(meas, hem, trk_sz, dir_in, regions,
  ...
  # Index uncorrected and FDR-corrected regions and behavioral measures by brain measure and hemisphere
  # and their corresponding uncorrected p-values
  for (fdr_cor in (0:1)){
    if (fdr_cor == 0){
      indx_cov_asym <- which(x_est_cov_asym < 2, arr.ind = T)
      indx_cov_r0 <- which(x_est_cov_r0 < 2, arr.ind = T)
      indx_asym <- which(x_est_asym < 2, arr.ind = T)
      indx_r0 <- which(x_est_r0 < 2, arr.ind = T)
    } else if (fdr_cor == 1){
    ...
    if (fdr_cor == 0){
      if (nrow(indx_cov_asym_txt) > 0){
        ...
        indx_cov_asym_txt[,3] <- x_est_cov_asym[indx_cov_asym]
      ...
      if (nrow(indx_cov_r0_txt) > 0){
        ...
        indx_cov_r0_txt[,3] <- x_est_cov_r0[indx_cov_r0]
      ...
      if (nrow(indx_asym_txt) > 0){
        ...
        indx_asym_txt[,3] <- x_est_asym[indx_asym]
      ...
      if (nrow(indx_r0_txt) > 0){
        ...
        indx_r0_txt[,3] <- x_est_r0[indx_r0]
        indx_r0_txt[,4] <- 'r0'
      }
    
    # consolidate estimates for ggseg visualizations
      indx_est <<- rbind(indx_cov_asym_txt, indx_cov_r0_txt, indx_asym_txt, indx_r0_txt)
