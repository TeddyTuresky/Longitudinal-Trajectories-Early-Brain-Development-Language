graph_all_meas_regs <- function(dir_in, measures, hem, trk){
  ...
  if (exists('struct_yes') == 1 & exists('dwi_yes') == 1){
    dat1.typ.all.mod <- rbind(dat1.typ.struct, dat1.typ.dwi.all3)
    graph_color = c('#00CC66', '#0033FF')
  } else if (exists('struct_yes') == 1 & exists('dwi_yes') == 0){
    dat1.typ.all.mod <- dat1.typ.struct
    graph_color = '#0033FF'
  } else if (exists('struct_yes') == 0 & exists('dwi_yes') == 1){
    dat1.typ.all.mod <- dat1.typ.dwi.all3
    graph_color = '#00CC66'
  }
  ...
  print(ggplot(dat1.typ.all.change, aes(x = age_act, y = Percent_Change, group = group, color = modality)) + 
          geom_path(lwd = 1, lty=1) + 
          scale_x_continuous(name = "Age (Months)", breaks = seq(0, 125, 25)) +
          scale_y_continuous(name = 'Change (%)', breaks = seq(0, 300, 50), limits = c(0, 300)) +  
          scale_color_manual(values = graph_color) + 
