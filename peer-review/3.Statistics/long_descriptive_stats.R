long_descriptive_stats <- function(dir_in, measures, hem, trks, covariates, beh_names, med_names, use_inf){
...
for (meas in measures){
  ...
  while (i == 0){
    ...
    dat <- dat[, c('id', 'timepoint', 'timepoint_n', 'age', 'pre_beh_age', 'beg_beh_age', covariates, beh_names, med_names)]
    if (meas == measures[1]){
      dat_all_meas <- dat 
    } else {
      dat_all_meas <- full_join(dat_all_meas, dat, by = c('id', 'timepoint', 'timepoint_n', 'age', 'pre_beh_age', 'beg_beh_age', covariates, beh_names, med_names))
    }
  }
}  

# retain necessary columns
dat1 <- dat_all_meas[, c('id', 'timepoint', 'timepoint_n', 'age', 'pre_beh_age', 'beg_beh_age', covariates, beh_names, med_names)]
...
colnames(mat_covs_beh) <- c('age', 'pre_beh_age', 'beg_beh_age', covariates, beh_names, med_names, 'n')
print(mat_covs_beh)
