long_graph_wcal <- function(dir_in, measures, hem, trks, covariates, use_inf){
...
for (meas in measures){
  ...
  while (i == 0){
    ...
    dat <- dat[, c('id', 'timepoint', 'timepoint_n', 'age', covariates)]
    if (meas == measures[1]){
      dat_all_meas <- dat 
    } else {
      dat_all_meas <- full_join(dat_all_meas, dat, by = c('id', 'timepoint', 'timepoint_n', 'age',  covariates))
    }
  }
}
...
dat1 <- dat_all_meas[, c('id', 'timepoint', 'timepoint_n', 'age', covariates)]

dat1$age <- as.numeric(dat1$age)
dat1$Sex <- as.factor(dat1$Sex)
if (any(grepl('cohort', covariates)) == 1){
  dat1$cohort <- as.factor(dat1$cohort)
} 
...
dat2_cov <- dat2[, c('id', 'timepoint', 'timepoint_n', covariates)]
dat2_cov <- na.omit(dat2_cov)
dat2 <- inner_join(dat2, dat2_cov, by = c('id', 'timepoint', 'timepoint_n', covariates))
...
# reorder data by age at earliest timepoint
dat2_earliest <- dat2 %>%
  group_by(id) %>%
  summarize(youngest_age = min(age))
dat2 <- left_join(dat2, dat2_earliest, by = 'id')
dat2 <- dat2[order(dat2$youngest_age, dat2$id), ]
...
print(ggplot(dat2, aes(x = age, y = as.numeric(reorder(id_num, youngest_age)), color = cohort)) + # to order by number of time points, change 'age' to 'n'
  scale_color_manual(values=c("deepskyblue3", "#ff8c00")) +
  geom_line(aes(group = id_num), linewidth = 0.2) +
  geom_point(aes(group = id_num), size = 0.3) +
  scale_x_continuous(name = "Age (Months)") +
  scale_y_continuous(name = 'Participant ID', breaks = seq(0, 140, by = 20), limits = c(-2, length(unique(dat2$id_num)) + 4), expand=c(0,0)) + 
