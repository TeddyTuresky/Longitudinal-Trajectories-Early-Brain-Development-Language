diff_node_clust <- function(diff_clus_elem, stats_sum, meas, hem, trk, dir_in, beh_names){
  ...
  if (length(dat4_int_mat1) < 100){
    print('some node values missing')
    dat4_int_mat1[c((length(dat4_int_mat1) + 1):100)] <- list(NULL)
  }
  
  if (length(dat4_slope_mat1) < 100){
    print('some node values missing')
    dat4_slope_mat1[c((lengthdat4_slope_mat1 + 1):100)] <- list(NULL)
  }
  
  if (length(dat4_50mo_mat1) < 100){
    print('some node values missing')
    dat4_50mo_mat1[c((length(dat4_50mo_mat1) + 1):100)] <- list(NULL)
  }
  
  if (length(dat2.typ.low_tract) < 100){
    print('some node values missing')
    dat2.typ.low_tract[c((length(dat2.typ.low_tract) + 1):100)] <- list(NULL)
  }
  
  if (length(dat2.typ.high_tract) < 100){
    print('some node values missing')
    dat2.typ.high_tract[c((length(dat2.typ.high_tract) + 1):100)] <- list(NULL)
  }
  
  if (length(dat2.typ.mid_tract) < 100){
    print('some node values missing')
    dat2.typ.mid_tract[c((length(dat2.typ.mid_tract) + 1):100)] <- list(NULL)
  }
  ...
  for (feat in c('int', 'slope', '50mo')){
    ...
    for (j in (1:length(beh_names))){
      ...
      if (s > 1){
        ...
        write.table(sig_nodes+5, paste0(dir_in, '/', hem, '_', meas, '_', trk, '_', beh_names[j], '_', feat, '_sig_nodes.txt'), 
        ...
        for (k in sig_nodes){
          if (k == sig_nodes[1]){
            big_dat1 <- data.frame(id = dat1_copy[[k+5]]$id, age = dat1_copy[[k+5]]$age,
                                   age_act = dat1_copy[[k+5]]$age_act, brain1 = dat1_copy[[k+5]]$brain1)
            ...
            df <- data.frame(id = dat1_copy[[k+5]]$id, age = dat1_copy[[k+5]]$age,
                             age_act = dat1_copy[[k+5]]$age_act, brain1 = dat1_copy[[k+5]]$brain1)
...
# ensure 100 nodes, fill with NULL
if (length(dat4_asym_mat1) < 100){
  print('some node values missing')
  dat4_asym_mat1[c((length(dat4_asym_mat1) + 1):100)] <- list(NULL)
}

if (length(dat4_r0_mat1) < 100){
  print('some node values missing')
  dat4_r0_mat1[c((length(dat4_r0_mat1) + 1):100)] <- list(NULL)
}
